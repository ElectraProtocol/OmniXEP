...
`test/functional/test_framework/test_framework.py:XepTestFramework.main`.
...
def main():
    ...
    enable_xepd = config["components"].getboolean("ENABLE_XEPD")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if xepd is already running
    # pidof might fail or return an empty string if xepd is not running
    try:
...
class RPCCoverage():
    ...
    commands per `xep-cli help` (`rpc_interface.txt`).
