...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\omnixep
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\omnixep
    // Mac: ~/Library/Application Support/omnixep
    // Unix: ~/.omnixep
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "omnixep";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/omnixep";
#else
    // Unix
    return pathRet / ".omnixep";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", XEP_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Electra Protocol") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Electra Protocol developers";
    }
