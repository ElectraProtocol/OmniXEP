...
{
    // Call into TxToUniv() in xep-common to decode the transaction hex.
    //
    // Blockchain contextual information (confirmations and blocktime) is not
    // available to code in xep-common, so we query them here and push the
                             ...
                             {RPCResult::Type::ARR, "vout", "",
                             {
                                 {RPCResult::Type::OBJ, "", "",
                                 {
                                     ...
                                     {RPCResult::Type::OBJ, "scriptPubKey", "",
                                     {
                                         ...
                                         {
                                             {RPCResult::Type::STR, "address", "xep address"},
                                         }},
                                     }},
                                 }},
                             }},
                ...
                "it is not stored in the wallet or transmitted to the network.\n",
                {
                    ...
                    {"outputs", RPCArg::Type::ARR, RPCArg::Optional::NO, "The outputs (key-value pairs), where none of the keys are duplicated.\n"
                            ...
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
                                    {"address", RPCArg::Type::AMOUNT, RPCArg::Optional::NO, "A key-value pair. The key (string) is the xep address, the value (float or string) is the amount in " + CURRENCY_UNIT},
                                },
                                },
                            {"", RPCArg::Type::OBJ, RPCArg::Optional::OMITTED, "",
                                {
...
static UniValue decodescript(const JSONRPCRequest& request)
{
    ...
    if (typeStr != "scripthash"  && typeStr != "scripthash_replay") {
        ...
        if (typeStr == "pubkey" || typeStr == "pubkey_replay" || typeStr == "pubkey_data_replay" || typeStr == "pubkeyhash" || typeStr == "pubkeyhash_replay" || typeStr == "multisig" || typeStr == "multisig_replay" || typeStr == "multisig_data" || typeStr == "multisig_data_replay" || typeStr == "nonstandard") {
            ...
            if (which_type == TX_PUBKEY || which_type == TX_PUBKEY_REPLAY || which_type == TX_PUBKEY_DATA_REPLAY || which_type == TX_MULTISIG || which_type == TX_MULTISIG_REPLAY || which_type == TX_MULTISIG_DATA || which_type == TX_MULTISIG_DATA_REPLAY) {
            ...
            CScript segwitScr;
            if (which_type == TX_PUBKEY || which_type == TX_PUBKEY_REPLAY || which_type == TX_PUBKEY_DATA_REPLAY) {
                segwitScr = GetScriptForDestination(WitnessV0KeyHash(Hash160(solutions_data[0].begin(), solutions_data[0].end())));
            } else if (which_type == TX_PUBKEYHASH  || which_type == TX_PUBKEYHASH_REPLAY) {
...
UniValue decodepsbt(const JSONRPCRequest& request)
{
            RPCHelpMan{"decodepsbt",
                "\nReturn a JSON object representing the serialized, base64-encoded partially signed Xep transaction.\n",
                {
                            ...
                            {RPCResult::Type::OBJ, "", "",
                            {
                                ...
                                {RPCResult::Type::OBJ, "witness_utxo", /* optional */ true, "Transaction output for witness UTXOs",
                                {
                                    ...
                                    {RPCResult::Type::OBJ, "scriptPubKey", "",
                                    {
                                        ...
                                        {RPCResult::Type::STR_HEX, "hex", "The hex"},
                                        {RPCResult::Type::STR, "type", "The type, eg 'pubkeyhash'"},
                                        {RPCResult::Type::STR, "address"," Xep address if there is one"},
                                    }},
                                }},
...
UniValue combinepsbt(const JSONRPCRequest& request)
{
            RPCHelpMan{"combinepsbt",
                "\nCombine multiple partially signed Xep transactions into one transaction.\n"
