...
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", XEP_PID_FILENAME)));
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location. (default: %s)", XEP_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-maxreorgdepth=<n>", strprintf("Configure at what depth blocks are considered final (default: %i). Use -1 to disable.", DEFAULT_MAX_REORG_DEPTH), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", XEP_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-experimental-xep-balances", strprintf("Maintain a full address index (default: %u)", DEFAULT_ADDRINDEX), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    gArgs.AddArg("-rpcforceutf8", "Replace invalid UTF-8 encoded characters with question marks in RPC responses (default: 1)", false, OptionsCategory::RPC);
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/ElectraProtocol/OmniXEP>";
...
/** Sanity checks
 *  Ensure that Xep is running in a usable environment with all
...
{
    // Make sure only a single Xep process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    fs::path config_file_path = GetConfigFile(gArgs.GetArg("-conf", XEP_CONF_FILENAME));
    ...
    if (gArgs.GetArg("-omniuseragent", true)) {
        ...
        if (gArgs.GetBoolArg("-experimental-xep-balances", DEFAULT_ADDRINDEX)) {
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            if (fAddressIndex != gArgs.GetBoolArg("-experimental-xep-balances", DEFAULT_ADDRINDEX)) {
                strLoadError = _("You need to rebuild the database using -reindex-chainstate to change -experimental-xep-balances").translated;
                break;
            }
    ...
    if (!gArgs.GetBoolArg("-txindex", DEFAULT_TXINDEX)) {
        ...
        msg += ": " + GetConfigFile(gArgs.GetArg("-conf", XEP_CONF_FILENAME)).string() + "\n\n";
        ...
        if (fRet) {
            ...
            fs::path configPathInfo = GetConfigFile(gArgs.GetArg("-conf", XEP_CONF_FILENAME));
            FILE *fp = fopen(configPathInfo.string().c_str(), "at");
            if (!fp) {
                std::string failMsg = _("Unable to update configuration file at").translated;
                failMsg += ":\n" + GetConfigFile(gArgs.GetArg("-conf", XEP_CONF_FILENAME)).string() + "\n\n";
